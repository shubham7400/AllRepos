...
import com.blueduck.annotator.util.formatFileSize
import com.blueduck.annotator.util.getDate

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun FileDetailsScreen(navController: NavHostController, viewModel: HomeViewModel) {
            ...
            Column(modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp).background(shape = MaterialTheme.shapes.medium, color = MaterialTheme.colorScheme.surfaceColorAtElevation(4.dp), )) {
                ...
                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    Column(modifier = Modifier.fillMaxWidth().padding(all = 16.dp).weight(weight = 1f)) {
                        ...
                        Text(text = file?.createdAt!!.getDate(Constant.FULL_DATE_FORMAT), style = MaterialTheme.typography.bodyMedium)
                    }
                    Column(modifier = Modifier.fillMaxWidth().padding(all = 16.dp).weight(weight = 1f)) {
                ...
                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    Column(modifier = Modifier.fillMaxWidth().padding(all = 16.dp).weight(weight = 1f)) {
                        ...
                        Text(text = file?.size!!.toLong().formatFileSize() , style = MaterialTheme.typography.bodyMedium)
                    }
                    Column(modifier = Modifier.fillMaxWidth().padding(all = 16.dp).weight(weight = 1f)) {}
                }
            }
