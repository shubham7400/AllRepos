...
object AppModule {
    ...
    @Provides
    fun provideProductsPagingSource(queryProductsByName: Query): ProductsPagingSource {
        return ProductsPagingSource(queryProductsByName = queryProductsByName)
    }

    @Provides
    fun providePagingConfig(): PagingConfig {
        return PagingConfig(pageSize = PAGE_SIZE.toInt())
    }

    @Provides
    fun provideProductsRepository(source: ProductsPagingSource, config: PagingConfig): ProductsRepository {
        return ProductsRepositoryImpl(source = source, config = config)
    }
}